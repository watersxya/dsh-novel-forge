...
| **修订仲裁（同一章只改一轮）**：审稿意见 + 时间线矛盾 + 张力偏差合并成一份指令，优先级 `审稿 high > 时间线 high > 张力 medium > 其它`，同源去重、单轮最多 8 条（截断显式声明）、每章最多 2 轮；张力/时间线**默认只提示不阻塞**（出章即核对 → 日志 + 待办），仅在因审稿 high 修订时搭同一轮车，复核基准覆盖全部下发条目 | **Revision arbitration**: review + timeline + tension merged into ONE instruction per chapter, fixed priority, dedup, 8-item cap with explicit truncation, 2 rounds max; tension/timeline are advisory-only (checked on chapter completion → log + todos) and ride along only when a review-driven revision happens |
