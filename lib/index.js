...
function renderAntiAiRules(assets) {
	...
	const isEncourage = (r) => r.severity === "encourage" || r.name.startsWith("鼓励") || r.avoid.startsWith("（鼓励类）");
	...
	if (forbidden.length > 0) {
		...
		for (const r of forbidden) lines.push(`- ${r.name}：避免——${clip(r.avoid, 90)}${r.fix !== "" ? `；修正——${clip(r.fix, 50)}` : ""}`);
	}
...
const REVIEW_DIMENSION_IDS = /* @__PURE__ */ new Set([
	"character",
	"setting",
	"redline",
	"writing",
...
function importDefaultAuthorAssets() {
	...
	const builtinKinds = /* @__PURE__ */ new Set([
		"genre",
		"antiAi",
		"progression",
		"style",
