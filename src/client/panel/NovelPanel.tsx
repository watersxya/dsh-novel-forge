...
/**
 * 面板宽度低于此值时自动折叠左侧导航。
 * 依据：.panelNav 占 216px，内容区留到 ~680px 才不至于把正文挤成竖排。
 * 宿主中列宽度 ≠ 视口宽度（用户可拖拽侧边栏），所以必须用实测宽度而非媒体查询。
 */
...
/** ⌘K 跳转映射：内容已搬进右滑抽屉的 tab → 抽屉 id（不在表内的走底座/浮窗分支）。 */
const CMD_DRAWER_OF: Partial<Record<NovelTab, 'write' | 'setup' | 'assets' | 'params' | 'overview' | 'blurb' | 'plotlines' | 'director' | 'knowledge' | 'run' | 'breakdown'>> = {
  plan: 'write',
  book: 'setup',
  assets: 'assets',
  overview: 'overview',
...
interface ChapterWorkbenchProps {
  ...
  onApprove: () => void
  onEdit: () => void
  /** 按作者勾选的审稿意见修订（不传则内部自动挑 high）。 */
  onRevisePicked: (issues: ReviewReport['issues']) => void
  onReset: () => void
  onRegenerate: () => void
  onBackfillAuthor: () => void
  /** 右栏当前页签（受控：修订完成后由外层自动切到「对比」）。 */
  tab: ChapterSideTab
  onTabChange: (tab: ChapterSideTab) => void
  /** 草稿审查报告（验证模式产物，针对草稿本身）；null = 还没审查过。 */
  draftReport: ReviewReport | null
  onApplyDraft: () => void
  onDiscardDraft: () => void
  onCheckDraft: () => void
}
...
function ChapterWorkbench({
  ...
  const author = chapter.authorReview
  const hasDraft = draft !== undefined && draft !== ''
  const notWritten = chapter.status === 'pending' || chapter.status === 'generating'
  /** 已有落盘正文（生成为异步，只有这三个状态才有正文可读、可处置）。 */
  const hasBody = chapter.status === 'written' || chapter.status === 'rejected' || chapter.status === 'approved'
  /** 可处置：有正文即可，不再要求 rejected —— 已通过的章也允许推翻重写 / 去AI味。 */
  const disposable = hasBody && text !== ''
  ...
  useEffect(() => {
    ...
    const medium = review.issues.map((it, i) => ({ it, i })).filter(x => x.it.severity === 'medium').map(x => x.i)
    setPicked(high.length > 0 ? high : medium)
    // 只依赖「报告签名」而非 review 对象引用：刷新不会打断作者的手选结果。
  }, [issueSig])
  /** 处置动作（跟随当前页签）：页签决定「看什么」，动作条决定「能做什么」。 */
  const footActions: ReactElement[] = []
  if (tab === 'review') {
    if (disposable) {
      footActions.push(
        <button key="rv" type="button" className={`${css.button} ${css.buttonSmall}`} disabled={busy || busyAny} onClick={onReview} title="重新跑一次 AI 审稿，覆盖现有报告">
          审稿 / 重审
        </button>
      )
    }
    if (canRevise && review !== undefined) {
      const r = review
      footActions.push(
        <button
          key="fix"
    ...
    if (disposable) {
      footActions.push(
        <button key="polish" type="button" className={`${css.button} ${css.buttonSmall}`} disabled={busy || busyAny} onClick={onEdit} title="打开工作区：指令框可填「去AI味 / 去套话」，左栏原文可选中局部处理">
          去AI味
        </button>
      )
      footActions.push(
        <button key="regen" type="button" className={`${css.button} ${css.buttonSmall}`} disabled={busy || busyAny} onClick={onRegenerate} title="整章重新生成，覆盖现有正文与审稿报告">
          重新生成
        </button>
      )
    }
    ...
    if (hasDraft) {
      footActions.push(
        <button
          ...
          title="把草稿写回正文并落盘（原稿自动备份 .bak）；沿用草稿审查结论更新章节状态"
        >
          ✓ 应用草稿
        </button>
      )
      footActions.push(
        <button key="discard" type="button" className={`${css.button} ${css.buttonSmall}`} disabled={busy || busyAny} onClick={onDiscardDraft} title="丢弃这份草稿，原稿一字不动">
          ↩ 放弃草稿
        </button>
      )
      footActions.push(
        <button
          ...
          title="对草稿跑一次 AI 审查（验证模式：核对原意见是否解决 + 只挑新增 high），不落盘"
        >
          {draftReport === null ? 'AI 审查草稿' : '重新审查草稿'}
        </button>
      )
    }
    if (disposable) {
      footActions.push(
        <button key="edit" type="button" className={`${css.button} ${css.buttonSmall}`} disabled={busy || busyAny} onClick={onEdit} title="打开工作区：手动微调 / 选中局部修订 / 整章重写">
          在工作区微调
        </button>
      )
    }
  } else if (author === undefined && hasBody) {
    footActions.push(
      <button key="bf" type="button" className={`${css.button} ${css.buttonSmall}`} disabled={busy} onClick={onBackfillAuthor} title="对该章补跑一次作者复盘（读取已落盘正文，不重新生成）">
        生成作者复盘
      </button>
    )
  }
  if (author !== undefined && tab === 'author') {
    footActions.push(
      <button key="bf2" type="button" className={`${css.button} ${css.buttonSmall}`} disabled={busy} onClick={onBackfillAuthor} title="读取最新正文重跑复盘，覆盖当前结论">
        重跑复盘
      </button>
    )
  }
    ...
    <div className={css.chWork}>
      <div className={css.chWorkBar}>
        ...
        <div className={css.chWorkActions}>
          ...
          {chapter.status === 'generating' && (
            <button type="button" className={`${css.button} ${css.buttonSmall}`} disabled={busy} onClick={onReset} title="生成卡死 / 中断时可复位为待生成，重新生成">
              复位
            </button>
          )}
          {hasBody && chapter.status !== 'approved' && (
            <button type="button" className={`${css.button} ${css.buttonSmall}`} disabled={busy || busyAny} onClick={onApprove} title="作者行使最终决定权：直接通过（不重审，保留审稿记录，落盘保存）">
              ✔ 直接通过
            </button>
          )}
        </div>
      ...
      <div className={css.chWorkGrid}>
        ...
        <main className={css.chWorkMain}>
          <div className={css.chWorkToolbar}>
            ...
            <span className={css.chWorkToolActions}>
              {disposable && (
                <button type="button" className={`${css.button} ${css.buttonSmall}`} disabled={busy || busyAny} onClick={onEdit} title="打开工作区：手动编辑正文 → AI 审查 → 保存">
                  编辑
                </button>
              )}
              {disposable && (
                <button type="button" className={`${css.button} ${css.buttonSmall}`} disabled={busy || busyAny} onClick={onEdit} title="打开工作区去AI味：指令框针对表达层面（去套话 / 去AI腔），左栏可选中局部处理">
                  去AI味
                </button>
              )}
            </span>
        ...
        <aside className={css.chWorkSide}>
          ...
          <div className={css.chWorkSideBody}>
                ...
                <div className={css.chWorkDraftBar}>
                  <div className={css.row} style={{ justifyContent: 'space-between' }}>
                    ...
                    <span className={css.meta}>
                      ...
                      （{(draft?.length ?? 0) - text.length >= 0 ? '+' : ''}{(draft?.length ?? 0) - text.length}）
                    </span>
                  </div>
                  {draftReport !== null ? (
                    <div className={css.meta}>
                      草稿审查：
                      <span style={{ color: draftReport.passed ? 'var(--nf-success)' : 'var(--nf-error)' }}>
                        {draftReport.score} 分 — {draftReport.passed ? '通过 ✓' : '未通过'}
                      </span>
                      {` · ${draftReport.verdict}`}
                      {draftReport.issues.length > 0 && ` · 剩 ${draftReport.issues.filter(i => i.severity === 'high').length} 条 high（共 ${draftReport.issues.length} 条）`}
                    </div>
                  ) : (
                    <div className={css.meta}>还没审查过这份草稿，点下方「AI 审查草稿」看有没有修好。</div>
                  )}
                </div>
                <DiffList original={text} draft={draft ?? ''} fontSize={fontSize} />
              </>
            ) : (
              <div className={css.meta}>
                本章还没有草稿。到「审稿」页勾选意见后点「按意见修订」，改完会自动跳回这里看改了什么。
              </div>
            ))}
                  ...
                  <>
                    <div className={css.chWorkPickBar}>
                      ...
                      <span className={css.chWorkPickBtns}>
                        ...
                        <button type="button" className={`${css.button} ${css.buttonSmall}`} onClick={() => { setPicked(review.issues.map((it, i) => ({ it, i })).filter(x => x.it.severity === 'high').map(x => x.i)) }} title="只勾选 high 级问题">
                          只选 high
                        </button>
                        <button type="button" className={`${css.button} ${css.buttonSmall}`} onClick={() => { setPicked([]) }} title="清空勾选">
                          清空
                        </button>
                      </span>
                    </div>
            ...
            {tab === 'author' && (author !== undefined ? (
              <div className={css.reviewBox} style={{ borderColor: 'color-mix(in srgb, var(--nf-info) 45%, transparent)' }}>
                ...
                {author.hookNote !== '' && <div className={css.meta}><b>钩子：</b>{author.hookNote}</div>}
                {author.plotlineProgress !== '' && <div className={css.meta}><b>推进：</b>{author.plotlineProgress}</div>}
                {author.continuity !== '' && <div className={css.meta}><b>衔接：</b>{author.continuity}</div>}
                {author.trend !== '' && <div className={css.meta}><b>趋势：</b>{author.trend}</div>}
              </div>
            ) : (
              <div className={css.row}>
                {chapter.status !== 'pending' && chapter.status !== 'generating' && chapter.status !== 'error' && (
                  <button
                    type="button"
                    className={`${css.button} ${css.buttonSmall}`}
                    disabled={busy}
                    onClick={onBackfillAuthor}
          ...
          {footActions.length > 0 && (
            <div className={css.chWorkSideFoot}>
              <div className={css.chWorkFootRow}>{footActions}</div>
              <span className={css.meta}>
                {tab === 'review'
                  ? '修订 / 去AI味 / 重新生成都会先进工作区预览，确认后才写回原稿。'
                  : tab === 'diff'
                    ? '草稿只有一份，新一轮修订会直接覆盖它。应用后原稿自动备份 .bak，放弃则原稿一字不动。'
                    : '复盘只读取已落盘正文，不重新生成正文。'}
              </span>
            </div>
          )}
...
export function NovelPanel({ controller, api }: NovelPanelProps) {
  ...
  /** v4 B3 主从布局：章节树搜索过滤 + 详情面板当前页签。 */
  const [chapterFilter, setChapterFilter] = useState('')
  const [chapterDetailTab, setChapterDetailTab] = useState<'beats' | 'text' | 'review' | 'author'>('text')
  ...
  const [bookCfgBusy, setBookCfgBusy] = useState(false)
  const [bookCfgNotice, setBookCfgNotice] = useState('')
  ...
  /** 角色卡（从事实库聚合）。 */
  const [, setCharCards] = useState<import('../../protocol.ts').RoleStatusCard[] | null>(null)
  ...
  const [, setNpmLatest] = useState<string | null>(null)
  ...
  useEffect(() => {
    ...
    let cancelled = false
    setBookCfgLoaded(false)
    api.bookSettings().then(r => {
      if (cancelled) return
      setBookCfg(r.settings)
      setBookCfgLoaded(true)
    }).catch(() => {
      if (!cancelled) setBookCfgLoaded(true)
    })
    return () => { cancelled = true }
  ...
  const saveBookCfg = async (): Promise<void> => {
    if (bookCfg === null) return
    setBookCfgBusy(true)
    setBookCfgNotice('')
    try {
      const patch: Record<string, unknown> = {}
      for (const key of BOOK_CFG_KEYS) patch[key] = bookCfg[key] ?? null
      const r = await api.bookSettings(patch)
      setBookCfg(r.settings)
      setBookCfgNotice('本书参数已保存')
      await refresh(false)
    } catch (err) {
      setBookCfgNotice((err as Error).message)
    } finally {
      setBookCfgBusy(false)
    }
  }
  /** 恢复全局默认：清空本书全部覆盖项。 */
  const resetBookCfg = async (): Promise<void> => {
    setBookCfgBusy(true)
    setBookCfgNotice('')
    try {
      const patch: Record<string, unknown> = {}
      for (const key of BOOK_CFG_KEYS) patch[key] = null
      const r = await api.bookSettings(patch)
      setBookCfg(r.settings)
      setBookCfgNotice('已恢复全局默认')
      await refresh(false)
    } catch (err) {
      setBookCfgNotice((err as Error).message)
    } finally {
  ...
  /** 模型快切：立即写入本书 provider/model 并刷新生效配置（选中即生效）。 */
  const quickSwitchModel = async (value: string): Promise<void> => {
    setBookCfgBusy(true)
    setBookCfgNotice('')
    try {
      let r: Awaited<ReturnType<typeof api.bookSettings>>
      if (value === 'global') {
        r = await api.bookSettings({ provider: null, model: null })
        setBookCfgNotice('本书模型已回退全局默认')
      } else {
        const idx = value.indexOf('::')
        const provider = value.slice(0, idx)
        const model = value.slice(idx + 2)
        r = await api.bookSettings({ provider, model })
        setBookCfgNotice(`本书模型已切换为 ${model}`)
      }
      setBookCfg(r.settings)
      await refresh(false)
    } catch (err) {
  ...
  /** R3 命令面板（⌘K / Ctrl+K）：命令表 = 全部 Tab 跳转 + 浮窗开关 + 设置。 */
  ...
  const cmdActions = useMemo<CmdAction[]>(() => [
    ...
    { icon: '', label: `跳转：${SETTINGS_TAB.label}`, hint: '底部', run: () => { openDrawer('params') } },
    { icon: '', label: '打开 AI 编辑 Agent 浮窗', run: () => { setAssistantOpen(true) } },
    { icon: '', label: '打开 AI 进度浮窗', run: () => { setProgressOpen(true) } },
  ], [])

  /** ⌘K / Ctrl+K 全局唤起；输入框聚焦时同样响应（面板本身是纯浏览入口）。 */
  useEffect(() => {
   ...
   *  水平方向保证窗口至少 80px 露在面板内，垂直方向不允许拖出顶/底。 */
  useEffect(() => {
    const onMove = (e: MouseEvent): void => {
      ...
      if (s.type === 'move') {
        ...
        const pw = panel?.clientWidth ?? 1200
        const ph = panel?.clientHeight ?? 900
        const next = {
          x: Math.max(80 - s.origW, Math.min(s.origX + e.clientX - s.startX, pw - 80)),
          y: Math.max(0, Math.min(s.origY + e.clientY - s.startY, ph - 40)),
        }
  ...
  /** 悬浮窗位置边界约束（与全局 mousemove 拖拽同一套规则）：
   *  水平保证窗口至少 80px 露在面板内，垂直不允许拖出顶/底。 */
  const clampFloatPos = useCallback((x: number, y: number, w: number, _h: number): { x: number; y: number } => {
    const panel = panelRef.current
    const pw = panel?.clientWidth ?? 0
    const ph = panel?.clientHeight ?? 0
    if (pw === 0 || ph === 0) return { x, y } // 面板未挂载/不可见时不猜测
    return {
      x: Math.max(80 - w, Math.min(x, pw - 80)),
      y: Math.max(0, Math.min(y, ph - 40)),
    }
  }, [])

  /** 启动时按面板实测尺寸矫正记忆的浮窗位置 —— 旧版本记录可能超出现在的边界规则。 */
  useEffect(() => {
    const panel = panelRef.current
    if (panel === null || panel.clientWidth === 0) return
    setAssistantPos(p => clampFloatPos(p.x, p.y, assistantSize.w, assistantSize.h))
    setProgressPos(p => clampFloatPos(p.x, p.y, progressSize.w, progressSize.h))
    // eslint-disable-next-line react-hooks/exhaustive-deps -- 仅启动时矫正一次
  }, [])

  /** 浮窗标题栏键盘移动：方向键 ±16px，Shift 微调 ±4px，Home 复位默认位置。 */
  const floatKeyDown = useCallback((target: 'assistant' | 'progress') => (e: ReactKeyboardEvent<HTMLDivElement>): void => {
    const fine = e.shiftKey ? 4 : 16
    let dx = 0
    let dy = 0
    if (e.key === 'ArrowLeft') dx = -fine
    else if (e.key === 'ArrowRight') dx = fine
    else if (e.key === 'ArrowUp') dy = -fine
    else if (e.key === 'ArrowDown') dy = fine
    else if (e.key === 'Home') {
      e.preventDefault()
      setFloatTop(target)
      const home = target === 'assistant' ? { x: 260, y: 60 } : { x: 60, y: 120 }
      const size = target === 'assistant' ? assistantSize : progressSize
      const next = clampFloatPos(home.x, home.y, size.w, size.h)
      if (target === 'assistant') setAssistantPos(next)
      else setProgressPos(next)
      return
    } else {
      return
    ...
    setFloatTop(target)
    const size = target === 'assistant' ? assistantSize : progressSize
    const setter = target === 'assistant' ? setAssistantPos : setProgressPos
    setter(p => clampFloatPos(p.x + dx, p.y + dy, size.w, size.h))
  }, [assistantSize, progressSize, clampFloatPos])
  ...
  const pushProgress = useCallback((text: string, kind: ProgressLine['kind'] = 'info') => {
    ...
    // R3 微交互：完成/失败给一次 Toast 确认（活动记录里照常留痕）
    if (kind === 'done') showToast(text, 'done')
    else if (kind === 'error') showToast(text, 'error')
  }, [])
  ...
  const handlePlotlineSuggest = async (): Promise<void> => {
    ...
    try {
      ...
      pushProgress(result.suggestions !== undefined && result.suggestions.length > 0
        ? `建议了 ${result.suggestions.length} 条剧情线，可逐条采纳`
        : 'AI 没有给出剧情线建议，请检查大纲是否已加载', 'done')
    } catch (err) {
      setError((err as Error).message)
      pushProgress(`建议剧情线失败：${(err as Error).message}`, 'error')
    } finally {
      setBusy(false)
      setBusyLabel('')
    }
  ...
  const handleRolesExtract = async (): Promise<void> => {
    ...
    try {
      ...
      pushProgress(`提炼出 ${fresh.length} 个新角色（已排除 ${(result.candidates?.length ?? 0) - fresh.length} 个已收录）`, 'done')
    } catch (err) {
      setError((err as Error).message)
  ...
  /** v4 B3 主从布局：选中章节 → 右侧详情，页签按章节阶段自动默认。 */
  const handleSelectChapter = async (no: number): Promise<void> => {
    setExpandedChapter(no)
    const ch = chapters.find(c => c.no === no)
    setChapterDetailTab(
      ch?.status === 'pending' || ch?.status === 'generating' || ch?.status === 'error'
        ? 'beats'
        : ch?.review !== undefined ? 'review' : 'text'
    )
    await loadChapterText(no)
  }

  /** 打开章节工作台（独立全页）：正文 + 对比 + 审稿 + 复盘 三栏并发，不做成抽屉/主从。 */
  const openWorkbench = async (no: number): Promise<void> => {
    setExpandedChapter(no)
    setWorkbenchNo(no)
    setWbTab('review')
    setWbDraftReport(null)
    await loadChapterText(no)
  }
  ...
  const checkDraftReport = async (no: number, previousReport?: ReviewReport): Promise<ReviewReport | null> => {
    const draft = chapters.find(c => c.no === no)?.pendingDraft
    if (draft === undefined || draft === '') return null
    setBusyLabel(`AI 审查草稿 第${no}章`)
    try {
      const result = await api.chapterCheck(no, draft, previousReport)
      // 主观项豁免：无 high 即视为通过，避免"越修越不通过"。
      const acceptable = reviseAcceptable(result.report)
      const report: ReviewReport = acceptable ? { ...result.report, passed: true } : result.report
      if (acceptable && !result.report.passed) {
        pushProgress(`草稿审查：${report.score} 分 — 可接受（剩余均为主观项，无逻辑 / 设定矛盾）`, 'done')
  ...
  const handleWbRevise = async (no: number, issues: ReviewReport['issues']): Promise<void> => {
    ...
    setError('')
    try {
      const previous = chapters.find(c => c.no === no)?.review
      await api.rewrite(no, instruction, '', frame => { applyJobFrame(frame, n => tt('progress.rewriting', { no: n })) })
      await refresh(false)
      setWbDraftReport(null)
      setWbTab('diff')
  ...
  /** 应用草稿落盘（原稿自动备份 .bak）→ 刷新正文 → 切回「审稿」看新报告。 */
  const handleWbApplyDraft = async (no: number): Promise<void> => {
    setBusy(true)
    setBusyLabel(`应用草稿 第${no}章`)
    setError('')
    try {
      await api.draftApply(no, wbDraftReport ?? undefined)
      await refresh(false)
      await loadChapterText(no)
      setWbDraftReport(null)
      setWbTab('review')
      pushProgress(`已应用草稿：第 ${no} 章已落盘（原稿已备份 .bak）`, 'done')
    } catch (err) {
      setError((err as Error).message)
      pushProgress(`应用草稿失败：${(err as Error).message}`, 'error')
    } finally {
      setBusy(false)
      setBusyLabel('')
    }
  ...
  /** 放弃草稿：保留原稿，仅清空草稿。 */
  const handleWbDiscardDraft = async (no: number): Promise<void> => {
    setBusy(true)
    setBusyLabel(`放弃草稿 第${no}章`)
    setError('')
    try {
      await api.draftDiscard(no)
      await refresh(false)
      setWbDraftReport(null)
      setWbTab('review')
      pushProgress(`已放弃草稿：第 ${no} 章原稿未改动`, 'done')
    } catch (err) {
      setError((err as Error).message)
      pushProgress(`放弃草稿失败：${(err as Error).message}`, 'error')
    } finally {
      setBusy(false)
      setBusyLabel('')
    }
  ...
  /** 手动重跑一次草稿审查（验证模式）。 */
  const handleWbCheckDraft = async (no: number): Promise<void> => {
    const previous = chapters.find(c => c.no === no)?.review
    setBusy(true)
    setError('')
    try {
  ...
  const gotoChapter = useCallback((no: number): void => {
    ...
    // v4 B3 主从布局：定位即选中，详情面板直接呈现该章。
    setExpandedChapter(no)
    void loadChapterText(no)
    setFocusNo(no)
  }, [chapters])
  ...
  const jumpToStage = (id: string): void => {
    switch (id) {
      case 'outline': openDrawer('overview'); break
      case 'bible': openDrawer('setup'); changeBookTab('bible'); break
      case 'volumes':
      case 'plan':
      case 'write': setActiveTab('plan'); break
      case 'review': openDrawer('setup'); changeBookTab('facts'); changeArchiveTab('reviews'); break
    }
  }
   ...
   * 覆盖全部可达页面：5 个资料格 + 工具页（生产单/知识库/拆书）。自动编辑与写作参数走顶栏入口。
   */
  const cabinetItems = (): Array<{ key: string; group: 'book' | 'tools'; name: string; di: string; dim?: boolean; go: () => void }> => [
    // —— 本书资料（全部右滑抽屉承载，底座总编台不被切走）——
    { key: 'outline', group: 'book', name: '总纲 / 大纲', di: project !== null ? `● ${(project.outline?.length ?? 0).toLocaleString()} 字` : '○ 未导入', dim: project === null, go: () => { openDrawer('overview') } },
    { key: 'bible', group: 'book', name: '道藏（设定 · 角色）', di: bible !== undefined ? `● ${bible.characters.length} 人 · ${bible.worldRules.length} 规则` : '○ 未提炼', dim: bible === undefined, go: () => { openDrawer('setup'); changeBookTab('bible') } },
    { key: 'reviews', group: 'book', name: '作者复盘', di: (() => { const r = chapters.filter(c => c.authorReview !== undefined); const h = r.filter(c => c.authorReview!.hookHonored).length; return r.length === 0 ? '○ 暂无' : `● 钩子兑现 ${h}/${r.length}` })(), dim: !chapters.some(c => c.authorReview !== undefined), go: () => { openDrawer('setup'); changeBookTab('facts'); changeArchiveTab('reviews') } },
    { key: 'plotlines', group: 'book', name: '剧情线 · 伏笔', di: (() => { const all = project?.plotlines ?? []; const a = all.filter(l => l.status === 'active').length; const p = foreshadows.filter(f => f.status === 'planned').length; return all.length === 0 && foreshadows.length === 0 ? '○ 暂无' : `● ${a} 线推进 · ${p} 笔待埋` })(), dim: (project?.plotlines ?? []).length === 0 && foreshadows.length === 0, go: () => { openDrawer('plotlines') } },
  ...
  return (
    <div ref={panelRef} className={css.panel} data-nf-mode={themeMode === 'system' ? undefined : themeMode} data-nf-density={themeDensity} style={panelBgStyle}>
      {viewMode === 'shelf' ? (
        ...
        <AuthorHome
          ...
          onChangeThemeDensity={changeThemeDensity}
        />
      ) : viewMode === 'create' ? (
        /* 开书向导：独立页面 */
        <CreateBookView
          ...
          <div className={css.panelTop}>
            ...
            <header className={css.panelHead}>
              <span
                ...
                onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); setViewMode('shelf') } }}
              >
                <span className={css.brandLogo}>书</span>
                <span className={css.brandText}>
                  <span className={css.brandName}>小说工坊<span className={css.brandEn}> · AI Novel Workbench</span></span>
                  <span className={css.brandSub}>
                    {activeBook !== null ? `${activeBook.bookName} · ${activeBook.done}/${activeBook.total} 章` : (project?.bookName !== undefined && project.bookName !== '' ? project.bookName : '未选书')}
                  </span>
                </span>
              </span>
              ...
              <button
                type="button"
                className={`${css.topNavBtn} ${progressOpen ? css.topNavActive : ''}`}
      ...
      {workbench !== null && (
        <ChapterWorkbench
          ...
          busy={busy}
          busyAny={busyAny}
          onBack={() => { setWorkbenchNo(null) }}
          onWrite={() => { void handleWriteChapter(workbench.no, true) }}
          onReview={() => { void handleReview(workbench.no) }}
          onApprove={() => { void handleChapterApprove(workbench.no) }}
          onEdit={() => { void openWorkspace(workbench.no) }}
          onRevisePicked={issues => { void handleWbRevise(workbench.no, issues) }}
          onReset={() => { void handleChapterReset(workbench.no) }}
          onRegenerate={() => { void handleWriteChapter(workbench.no, true) }}
          onBackfillAuthor={() => { void handleAuthorBackfillChapter(workbench.no) }}
          tab={wbTab}
          onTabChange={setWbTab}
          draftReport={wbDraftReport}
      ...
      {rightDrawer !== null && (
        <div className={css.drawerScrim} onClick={closeDrawer} aria-hidden="true" />
      )}
      ...
      {rightDrawer !== null && (
        <aside className={`${css.contentDrawer} ${drawerMax ? css.drawerMax : ''}`} role="dialog" aria-label="工作台抽屉">
                  ...
                  {group.chapters.map(chapter => {
                    ...
                    return (
                      <button
                        key={chapter.no}
                        type="button"
                        className={css.wbIndexRow}
                        data-chapter-no={chapter.no}
                        title={chapter.beats}
                        onClick={() => { closeDrawer(); void openWorkbench(chapter.no) }}
                      >
                        <span className={css.planDot} style={{ background: chapter.status === 'approved' ? 'var(--nf-success)' : chapter.status === 'rejected' || chapter.status === 'error' ? 'var(--nf-error)' : chapter.status === 'generating' ? 'var(--nf-info)' : chapter.status === 'written' ? 'var(--nf-accent)' : 'var(--nf-border-strong)' }} />
                        <span className={css.planTreeRowNo}>{chapter.no}</span>
                        <span className={css.wbIndexTitle}>{chapter.title || `第 ${chapter.no} 章`}</span>
                        <span className={`${css.badge} ${badge.cls}`}>{badge.label}</span>
                      </button>
                    )
                  })}
          ...
          <div className={css.card} style={{ padding: 0, overflow: 'hidden' }}>
            <div className={css.t2Master}>
              <aside className={css.t2Rail} aria-label="本书设定分区">
                <div className={css.t2RailHead}> 本书设定</div>
                {([
                  ['bible', '', '设定库'],
                  ['world', '', '大世界'],
                  ['roles', '', `角色库（${project?.roles?.length ?? 0}）`],
                  ['facts', '', '编年 / 复盘'],
                  ...
                  <button
                    ...
                    data-active={bookTab === id ? '' : undefined}
                    onClick={() => { changeBookTab(id) }}
                  >
                    <span>{icon}</span>
                    <span>{label}</span>
                  </button>
        ...
        {/* 角色库：全书角色主表（独立导航页） */}
          ...
          <div className={css.card}>
            ...
            <SlideNav
              ariaLabel="编年与复盘"
              items={[
                { id: 'facts', label: ` 编年录（${(project?.facts ?? []).length}）` },
                { id: 'reviews', label: ` 复盘记录（${chapters.filter(c => c.authorReview !== undefined).length} 章）` },
              ]}
              active={archiveTab}
              onSelect={changeArchiveTab}
            />
            ...
            {(project?.facts ?? []).length > 0 ? (
              ...
              <EmptyState compact icon="" title="暂无事实条目" hint="写一章后会自动生成，或点击上方「回填」。" />
            )}
              </>
            )}
          </div>
          ...
          <>
            <div className={`${css.card} ${css.settingsCard}`} style={{ gap: 'var(--nf-space-12)' }}>
              ...
              <span className={css.meta}>只对当前书生效，随书目录保存（换书各管各的，目录迁移自动跟随）；未设置的项回退全局默认（书架页「设置」）。</span>
              {bookCfgNotice !== '' && <span className={css.meta} style={{ color: 'var(--nf-success)' }}>{bookCfgNotice}</span>}
            </div>

            {!bookCfgLoaded ? (
              <div className={css.card}><span className={css.meta}>本书参数加载中…</span></div>
            ) : bookCfg === null ? (
              <div className={css.card}><span className={css.meta}>当前没有打开的书：先在书架开书或激活一本书，再设置本书参数。</span></div>
            ) : (
              <>
                {/* 模型快切：本书用模型库里的哪个模型（像聊天框一样自由切换） */}
                <div className={`${css.card} ${css.settingsCard}`} style={{ gap: 'var(--nf-space-12)' }}>
                  <span className={css.cardTitle}>模型快切（本书）</span>
                  <div className={css.field}>
                    <label className={css.fieldLabel}>本书模型</label>
                    <select
                      className={css.input}
                      disabled={bookCfgBusy}
                      value={(bookCfg.provider ?? '') !== '' && (bookCfg.model ?? '') !== '' ? `${bookCfg.provider}::${bookCfg.model}` : 'global'}
                      onChange={e => { void quickSwitchModel(e.target.value) }}
                    >
                      <option value="global">跟随全局（{config?.provider ?? ''} / {config?.model ?? ''}）</option>
                      {(configDraft?.savedModels ?? []).map(m => (
                        <option key={m.id} value={`${m.provider}::${m.model}`}>{m.name !== '' ? m.name : m.model}（{m.provider} / {m.model}）</option>
                      ))}
                    </select>
                    <span className={css.meta}>选中即切换、立即生效（下一章生成用新模型）；只切本书主力模型，生成/审稿/质检专项路由仍在书架页「设置 → 模型」。</span>
                    {(project?.chapters ?? []).some(c => c.status === 'written' || c.status === 'approved') && (
                      <span className={css.meta}>本书已有已写章节：中途换模型可能带来文风变化，建议在卷首或重写节点切换。</span>
                    )}
                  </div>
                </div>
                ...
                <div className={`${css.card} ${css.settingsCard}`} style={{ gap: 'var(--nf-space-24)' }}>
                  <span className={css.cardTitle}>写作与审稿（本书）</span>
                  <div className={css.field}>
                    <label className={css.fieldLabel}>{tt('settings.chapterChars')}</label>
                    <input className={css.input} type="number" min={1000} max={20000} value={bookCfg.chapterChars ?? ''} placeholder={`全局默认 ${config?.chapterChars ?? ''}`} onChange={e => { setBookCfg({ ...bookCfg, chapterChars: e.target.value === '' ? undefined : Number(e.target.value) }) }} />
                    <span className={css.meta}>留空 = 跟随全局默认。</span>
                  </div>
                  <div className={css.field}>
                    <label className={css.fieldLabel}>{tt('settings.maxTokens')}</label>
                    <input className={css.input} type="number" min={2000} max={64000} value={bookCfg.maxTokens ?? ''} placeholder={`全局默认 ${config?.maxTokens ?? ''}`} onChange={e => { setBookCfg({ ...bookCfg, maxTokens: e.target.value === '' ? undefined : Number(e.target.value) }) }} />
                    <span className={css.meta}>留空 = 跟随全局默认。</span>
                  </div>
                  <div className={css.field}>
                    <label className={css.fieldLabel}>{tt('settings.reviewPassScore')}</label>
                    <input className={css.input} type="number" min={0} max={100} value={bookCfg.reviewPassScore ?? ''} placeholder={`全局默认 ${config?.reviewPassScore ?? ''}`} onChange={e => { setBookCfg({ ...bookCfg, reviewPassScore: e.target.value === '' ? undefined : Number(e.target.value) }) }} />
                    <span className={css.meta}>通过判定：综合评分 ≥ 此分数（默认 70），或无 high 级问题且评分 ≥ 60；留空 = 跟随全局。</span>
                  </div>
                  <div className={css.field}>
                    <label className={css.fieldLabel}>{tt('settings.autoReview')}</label>
                    <select className={css.input} value={bookCfg.autoReview === undefined ? '' : (bookCfg.autoReview ? '1' : '0')} onChange={e => { setBookCfg({ ...bookCfg, autoReview: e.target.value === '' ? undefined : e.target.value === '1' }) }}>
                  ...
                  <div className={css.field}>
                    ...
                    <select className={css.input} value={bookCfg.autoAuthorReview === undefined ? '' : (bookCfg.autoAuthorReview ? '1' : '0')} onChange={e => { setBookCfg({ ...bookCfg, autoAuthorReview: e.target.value === '' ? undefined : e.target.value === '1' }) }}>
                      <option value="">跟随全局</option>
                      <option value="1">✓ 是</option>
                      <option value="0">✗ 否</option>
                    </select>
                  ...
                  <div className={css.field}>
                    ...
                    <select className={css.input} value={bookCfg.autoReviewAfterRevise === undefined ? '' : (bookCfg.autoReviewAfterRevise ? '1' : '0')} onChange={e => { setBookCfg({ ...bookCfg, autoReviewAfterRevise: e.target.value === '' ? undefined : e.target.value === '1' }) }}>
                      <option value="">跟随全局</option>
                      <option value="1">✓ 是</option>
                      <option value="0">✗ 否</option>
                    </select>
                ...
                {/* 推理力度（本书） */}
                <ReasoningSection
                  reasoningEffort={bookCfg.reasoningEffort ?? 'off'}
                  analysisReasoning={bookCfg.analysisReasoning ?? 'low'}
          ...
          {/* 写作资产 5 个子分类（左侧导航直达） */}
          {rightDrawer === 'assets' && <AssetsTab api={api} />}
          {rightDrawer === 'overview' && (
          <>
            <div className={css.card}>
              <PageHeader
                eyebrow="Overview"
                title={tt('tab.overview')}
              ...
              {project === null ? (
                <>
                  ...
                  <textarea
                    className={`${css.textarea} ${css.docsSurface} ${css.outlineEdit}`}
                    value={outlineText}
                ...
                <>
                  ...
                  <textarea
                    className={`${css.textarea} ${css.outlineEdit}`}
                    value={outlineText}
                ...
                /* 只读展示：大纲是开书时的出生证明（结构化稿纸阅读） */
                <OutlineDocument text={project.outline} />
              )}
            </div>
            <details className={css.card} open>
              <summary className={css.cardTitle}>
                {tt('status.files')}（{generatedFiles.length}）
              </summary>
          ...
          <div className={css.card}>
            ...
            <span className={css.meta}>
              面向读者的作品门面（120-250 字）：突出核心卖点与开局钩子，不剧透。点击 生成全量生成；或先写几句再点 补全续写完整；不满意可  重新生成。
            </span>
            {project === null ? (
              ...
              <>
                <textarea
                  className={css.textarea}
                  style={{ minHeight: 140 }}
                  placeholder="点击 生成，或先写下开头几句，再点 补全…"
                  value={blurbDraft}
          ...
          {rightDrawer === 'plotlines' && (
          <div className={css.card}>
            <PageHeader eyebrow="Plotlines" title="长线管理" />
            <SlideNav
              ariaLabel="长线管理分区"
              items={[
                { id: 'plotlines', label: ` 剧情线（${project?.plotlines?.length ?? 0}）` },
                { id: 'foreshadow', label: ` 伏笔（${foreshadows.length}）` },
              ]}
              active={longlineTab}
              onSelect={changeLonglineTab}
            />
      ...
      <div className={`${css.panelBody} ${workbench !== null ? css.panelHidden : ''}`}>
        ...
        {workspace !== null && (
          <div className={css.wsPage}>
            ...
            <div className={css.wsColumns}>
              ...
              <div className={css.wsColumn}>
                <div className={css.meta} style={{ fontWeight: 600 }}>修正指令</div>
                <textarea
                  className={css.textarea}
                  style={{ minHeight: 60 }}
                ...
                {wsCheckReport !== null && (
                  <div className={css.wsPreview} style={{ borderColor: wsCheckReport.passed ? 'var(--nf-success)' : 'var(--nf-warn)' }}>
                    <div className={css.busyRow}>
                      <span className={css.meta} style={{ fontWeight: 600 }}>审查结果</span>
                      <span style={{ color: wsCheckReport.passed ? 'var(--nf-success)' : 'var(--nf-error)' }}>
        ...
        {activeTab === 'workflow' && (
          <>
          <div className={css.wfStage}>
            {/* ── 顶栏：书名 · 总进度 · 当前工序 · 工具入口收到右上角 ── */}
            <div className={css.wfTop}>
              {/* 样稿 topbar 左侧：返回书架 */}
              <button type="button" className={css.subPageBack} onClick={() => { setViewMode('shelf') }} title="返回书架">
                ← 书架
              </button>
              <span className={css.wfBook} title={project !== null ? project.bookName : ''}>
                {project !== null ? `《${project.bookName}》` : '书'}
              </span>
              <span className={css.wfProg}>
                <b className={css.wfProgBig}>{approvedCount}</b>
                <span className={css.wfProgRest}>/ {chapters.length} 章过审</span>
              </span>
              {(() => {
                const now = journeyStages.find(s => s.id === currentStageId)
                const all = journeyStages.every(s => s.done)
                const label = all ? '已收尾' : (now?.label ?? '—')
                return (
                  <span className={css.wfPill} title={all ? '全部工序完成' : `当前处于「${label}」工序`}>
                    <i className={css.wfPillDot} style={{ background: 'var(--wf-ok)' }} />
                    {all ? '全书完工' : `${label}阶段`}
                  </span>
                )
              })()}
              {writingNow !== undefined && (
                <span className={`${css.wfPill} ${css.wfLiveChip}`} title={`AI 正在写第 ${writingNow.no} 章正文`}>
                  <i className={css.wfPillDot} style={{ background: 'var(--wf-ai)' }} />
                  AI 写作中 · 第 {writingNow.no} 章
                </span>
              )}
            ...
            <div className={css.wfBody}>
              ...
              <div className={css.wfColB}>
                ...
                <div className={`${css.wfBox} ${css.wfAccentAmber}`}>
                  ...
                  <div className={css.wfHint}>范围可框选「第 1–N 章」，AI 全自动跑：生成 → 摘要+事实 → 伏笔标记 → 审稿 → 复盘；被拒自动修订 2 轮。</div>
                  <div className={css.wfConsole}>
                    {(() => {
                      const lines: Array<{ txt: string; tone?: 'ok' | 'err'; live?: boolean; ratio?: number }> = []
                      const live = progress.find(p => p.live === true)
                      if (live !== undefined) {
                        lines.push({ txt: live.text, live: true, ratio: live.ratio })
                      }
                      const hist = progress.filter(p => p.live !== true).slice(-3).reverse()
                      for (const p of hist) {
                        lines.push({ txt: p.text, tone: p.kind === 'done' ? 'ok' : p.kind === 'error' ? 'err' : undefined })
                      }
                      if (lines.length === 0) {
                        lines.push({ txt: writingNow !== undefined ? `正在写第 ${writingNow.no} 章…` : '暂无运行记录 —— 点下方按钮让机器开跑' })
                      }
                      return lines.map((ln, idx) => (
                        <div key={idx} className={`${css.wfConRow} ${ln.tone === 'ok' ? css.wfConOk : ln.tone === 'err' ? css.wfConErr : ''}`}>
                          {ln.live === true && <span className={css.wfLiveMark}>▮</span>}
                          <span className={css.wfConTxt}>{ln.txt}</span>
                          {ln.live === true && ln.ratio !== undefined && (
                            <span className={css.wfConBar}><i style={{ width: `${Math.round(ln.ratio * 100)}%` }} /></span>
                          )}
                        </div>
                      ))
                    })()}
                  </div>
                  <div className={css.wfOpsRow}>
                    <button type="button" className={`${css.wfOpsBtn} ${css.wfOpsBtnMain}`} disabled={busy || pendingCount === 0} onClick={() => { void handleWriteAll() }}>
                      ▶ 让 AI 写完全部（剩 {pendingCount}）
                    </button>
                    <button type="button" className={css.wfOpsBtn} disabled={busy || doneCount === 0} onClick={() => { void handleAudit() }}> 全书质检</button>
                    <button type="button" className={css.wfOpsBtn} onClick={() => { openDrawer('run') }}> 生产单面板</button>
                  </div>
                  {auditStatus?.status === 'running' && (
                    <div className={css.wfConsoleNote}>质检中：{auditStatus.completedBatches}/{auditStatus.totalBatches} 批 · 机器正在跑，不影响你裁决</div>
                  )}
                </div>

                {/* 当前环节坐标 */}
                <div className={css.wfBox}>
                  <h5 className={css.wfBoxH}><i className={css.wfDot} style={{ background: 'var(--wf-idle)' }} />当前环节坐标</h5>
                  <div className={css.wfCoord}>
                    {writingNow !== undefined ? (
                      <>机器正在写：<b>第 {writingNow.no} 章</b> · {writingNow.title || '（未命名）'}</>
                    ) : pendingCount > 0 ? (
                      <>机器空闲：还有 <b>{pendingCount} 章</b>待生成，点上方「▶ 让 AI 写完全部」即开跑</>
                    ) : doneCount > 0 ? (
                      <>全部章节已写完，流水线空闲 —— 可全书质检或进入收尾工序</>
                    ) : (
                      <>还没有可写的章 —— 先完成卷计划 / 章节计划工序</>
                    )}
                    {(() => {
                      const stuck = chapters.filter(c => c.status === 'rejected' || c.status === 'error')
                      return stuck.length > 0 ? (
                        <div className={css.wfCoordStuck}>卡点：{stuck.slice(0, 3).map(c => `第${c.no}章`).join('、')} 需人工介入 → <button type="button" className={css.wfAct} onClick={() => { gotoChapter(stuck[0].no) }}>去处理</button></div>
                      ) : null
                    })()}
                  </div>
                </div>
        ...
        {activeTab === 'plan' && (
          <>
            ...
            {chapters.length > 0 && (
              <div className={css.card} style={{ padding: 0, overflow: 'hidden' }}>
                {/* 卷筛选已由左树卷折叠组承担，此处只保留「批量写待生成」动作。 */}
                {pendingCount > 0 && (
                  <div className={css.row} style={{ justifyContent: 'flex-end', flexWrap: 'wrap', padding: 'var(--nf-space-10) var(--nf-space-14) 0' }}>
                ...
                {/* v4 B3 主从布局：左列章节树（卷折叠 + 状态点 + 搜索），右列单章详情（页签 + 动作条吸底）。 */}
                <div className={css.planMaster}>
                  <aside className={css.planTree} aria-label="章节树">
                    <input
                      className={`${css.input} ${css.planTreeSearch}`}
                      type="search"
                    ...
                    {chapterGroups.filter(g => selectedVolume === 'all' || g.no === selectedVolume).map(group => {
                      ...
                      const filtered = chapterFilter.trim() === ''
                        ? group.chapters
                        : group.chapters.filter(c => c.title.includes(chapterFilter.trim()) || String(c.no).includes(chapterFilter.trim()))
                      if (chapterFilter.trim() !== '' && filtered.length === 0) return null
                      return (
                        <div key={group.no} className={css.planTreeVolGroup}>
                          <div
                            className={css.planTreeVol}
                            role="button"
                            tabIndex={0}
                            onClick={() => {
                              if (group.no !== 0) {
                          ...
                          >
                            <span>{group.no !== 0 ? (collapsed ? '▸' : '▾') : ''}</span>
                            <b>{group.title}</b>
                            <span className={css.planTreeVolMeta}>{groupDone}/{group.chapters.length}</span>
                          </div>
                          {!collapsed && filtered.map(chapter => {
                            const dot = chapter.status === 'approved' ? 'var(--nf-success)'
                              : chapter.status === 'rejected' || chapter.status === 'error' ? 'var(--nf-error)'
                              : chapter.status === 'generating' ? 'var(--nf-info)'
                              : chapter.status === 'written' ? 'var(--nf-accent)'
                              : 'var(--nf-border-strong)'
                            return (
                              <button
                                key={chapter.no}
                                type="button"
                                role="tab"
                                aria-selected={expandedChapter === chapter.no}
                                data-active={expandedChapter === chapter.no ? '' : undefined}
                                data-chapter-no={chapter.no}
                                className={css.planTreeRow}
                                title={chapter.beats}
                                onClick={() => { void handleSelectChapter(chapter.no) }}
                              >
                                <span className={css.planDot} style={{ background: dot }} />
                                <span className={css.planTreeRowNo}>{chapter.no}</span>
                                <span className={css.planTreeRowTitle}>{chapter.title}</span>
                              </button>
                            )
                          })}
                        </div>
                  ...
                  <section className={css.planDetail}>
                    {(() => {
                      const chapter = expandedChapter !== null ? chapters.find(c => c.no === expandedChapter) : undefined
                      if (chapter === undefined) {
                        return <div className={css.planDetailEmpty}><span className={css.meta}>← 从左侧选一章查看节拍 / 正文 / 审稿报告 / 复盘</span></div>
                      }
                      const badge = statusBadge(chapter)
                      const review: ReviewReport | undefined = chapter.review
                        ...
                        <>
                          ...
                          <SlideNav
                            ...
                            items={[
                              { id: 'beats', label: ' 节拍' },
                              { id: 'text', label: ' 正文' },
                              { id: 'review', label: ' 审稿报告' },
                              { id: 'author', label: ' 复盘' },
                            ]}
                            active={detailTab}
                            onSelect={setChapterDetailTab}
                          />
                          <div className={css.planDetailBody}>
                            {detailTab === 'beats' && (
                              <div className={css.planBeats}>
                                <div className={css.planBeatsLabel}>{tt('plan.beats')}</div>
                                <div className={css.meta}>{renderBeats(chapter.beats)}</div>
                                {chapter.summary !== undefined && chapter.summary !== '' && (
                                  <div className={css.meta}><b>{tt('plan.summary')}:</b> {chapter.summary}</div>
                                )}
                                {chapter.status === 'approved' && chapter.chars !== undefined && (
                                  <div className={css.meta}>{chapter.chars}{tt('common.chars')}{chapter.volume > 0 ? ` · ${tt('plan.volumes')}${chapter.volume}` : ''}</div>
                                )}
                              </div>
                            )}
                            {detailTab === 'text' && (
                          ...
                          <div className={css.planDetailBar}>
                            <button
                              type="button"
                              className={`${css.button} ${css.buttonSmall}`}
                              onClick={() => { void openWorkbench(chapter.no) }}
                              title="打开章节工作台：独立全页，正文 / 对比 / 审稿 / 复盘 三栏并发"
                            >
                              ⤢ 独立全页
                            </button>
                            {(chapter.status === 'pending' || chapter.status === 'error') && (
                            ...
                            {(chapter.status === 'rejected' || chapter.status === 'written' || chapter.status === 'approved') && (
                              <span className={css.meta} style={{ flexBasis: '100%' }}>
                                润色 / 修订 — 打开工作区：左栏原文可直接选中文字做局部修订，右栏输入指令后预览，确认后再应用（未应用不改动原稿）
                              </span>
                            )}
                          </div>
                        </>
      ...
      {assistantOpen && (
        <div
          ...
          data-top={floatTop === 'assistant' ? '' : undefined}
        ...
        >
          <div
            ...
            tabIndex={0}
            role="button"
            aria-label="移动 AI 编辑 Agent 窗口：方向键移动，Shift 加方向键微调，Home 复位"
            onKeyDown={floatKeyDown('assistant')}
            onMouseDown={e => {
              e.preventDefault()
              setFloatTop('assistant')
          ...
          >
            <span> AI 编辑 Agent <span className={css.meta}>（拖动或方向键移动 · 右下角拉大小）</span></span>
      ...
      {progressOpen && (
        <div
          className={css.assistantFloat}
          data-top={floatTop === 'progress' ? '' : undefined}
        ...
        >
          <div
            ...
            tabIndex={0}
            role="button"
            aria-label="移动 AI 进度窗口：方向键移动，Shift 加方向键微调，Home 复位"
            onKeyDown={floatKeyDown('progress')}
            onMouseDown={e => {
              e.preventDefault()
              setFloatTop('progress')
          ...
          >
            <span>
              ...
              <span className={css.meta}>（拖动或方向键移动 · 右下角拉大小）</span>
            </span>
...
function OutlineDocument({ text }: { text: string }): ReactElement {
  ...
  return (
    <div className={css.outlineDocWrap}>
      ...
      {toc.length > 1 && (
        <nav className={css.outlineToc} aria-label="大纲目录">
          {toc.map(t => (
            <button key={t.id} type="button" className={`${css.outlineTocItem} ${TOC_CLASS[t.level] ?? ''}`} onClick={() => scrollTo(t.id)}>{t.text}</button>
          ))}
        </nav>
      )}
      {volumes.length <= 1 ? (
